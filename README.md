...
- `MAVEN` and `MODULAR_TO_MAVEN` use `MavenRepositoryStaging`. For each main module it parses `prefix.pom`
  ...
  `~/.m2/repository`, overridable via `MAVEN_LOCAL_REPOSITORY`) with the right `maven-metadata-local.xml` and
...
`requires.properties` from the Java `requires` directives. Javadoc tags of the form `@pin <module> <version>`
...
/**
 * @release 25
 * @pin org.junit.jupiter 5.11.3
 * @pin org.junit.platform.commons 1.11.4
 */
...
An optional space-separated `<algorithm>/<hex>` after the version on a `@pin` Javadoc tag
(e.g. `@pin org.junit.jupiter 5.11.3 SHA256/abcdef0123...`) is captured into the same
...
- **`Repository.ofProperties(suffix, folders, resolver, [versionResolver,] cache)`** - bulk-loads a
  ...
  repository map, with `.jenesis/cache/` serving as the cross-build hardlink cache.
...
`.jenesis/cache/` folder). When `MAVEN_LOCAL_REPOSITORY` is set explicitly, the directory must exist or the
...
at a published Jenesis module repository works the same way. A `(URI, String token)` overload accepts an
explicit auth token for use with custom remote repositories: when non-null, the value is sent verbatim as
an `Authorization` header on every HTTP fetch (e.g. `Bearer <token>` or `Basic <base64(user:pass)>`). The
token is ignored for `file://` roots and any non-HTTP scheme. The no-arg constructor does not pick up a
token from the environment - tokens are an opt-in only used when wiring a custom `JenesisModuleRepository`
explicitly through `Project.repositories(...)`. A bare-name coordinate (`<module>`) fetches the
...
`~/.m2/repository` for the Maven layout. The default location is `~/.jenesis` (overridable via the
`JENESIS_MODULE_REPOSITORY` environment variable, honoured by both the export and the repository's no-arg
constructor); a `URI`-arg constructor on `JenesisModuleRepository` accepts any root, so a `file://`,
`http://`, or `https://` URL pointing at a publication of the same shape works equally well.
...
those files. A consumer that pins `@pin <module> <version>` always reads from the versioned subdirectory
...
through the versioned path after pinning - once a `@pin <module> <version>` lands in source, downstream
...
What lives there today: hardlinked jars fetched by the `MODULAR` layout's per-prefix
`Repository.ofUris(...).cached(.jenesis/cache)` wrappers (wired in `Project.Layout.MODULAR`). Each entry is
...
- **`ModuleInfoParser` / `ModuleInfo`** parse `module-info.java` via the `javax.tools` / `com.sun.source` APIs
  ...
  `requires static transitive`), and a `versions` map extracted from `@pin <module> <version>` Javadoc tags
...
- **`MavenRepositoryExport`** is the `BuildStep` that publishes the staged tree to an external Maven repository
  directory (default `~/.m2/repository`, overridable via the `MAVEN_LOCAL_REPOSITORY` environment variable).
...
`@pin` reads the description, so the cost is essentially free:
...
and `ModularProject.Manifests` flags any module whose `module-info.java` declares an `@test` Javadoc tag
(parsed by `ModuleInfoParser` into `ModuleInfo.testOf()`). Its value is the `artifactId` of the main module the
tests cover (or empty for the deprecated bare `@test` form, which only resolves when exactly one main module
