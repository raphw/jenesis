...
- `executed` (`TestModule.Run` extends `Java`) accepts `filter`, `group`, `parallel`, and `reporting` arguments.
  ...
  `group` is a comma-separated list of test groups; `parallel` and `reporting` are flags. `TestModule`
  resolves each of these from a `jenesis.test.*` system property (`jenesis.test.filter`, `jenesis.test.group`,
  `jenesis.test.parallel`, `jenesis.test.reporting`) as an implicit default, **unless** the corresponding
  `TestModule` wither explicitly set it - a wither always wins, so the property applies only when the value was
  left unset (the fields are nullable for exactly this reason). The assembler no longer reads these properties;
  it wires a bare `TestModule` and lets it resolve its own defaults. The
