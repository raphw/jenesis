...
| -------------------------- |------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| ------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| `Bind`                     | Hard-links files from each predecessor into a target layout under `context.next()`. Three modes: a `Map<Path, Path>` mirrors whole subtrees under canonical names (used by the static factories `asSources()`, `asResources()`, `asCoordinates(...)`, `asDependencies(...)`); a `Function<Path, Optional<Path>>` selects per file, optionally restricted to a `Set<Path>` of subtree prefixes for path-aware `shouldRun`. | a source folder, a named properties file, or arbitrary predecessor outputs                                                            | `sources/`, `resources/`, `coordinates.properties`, `dependencies.properties`, or whatever the placement function returns |
