...
- `MultiProjectAssembler<D extends ModuleDescriptor>` (set via `Project.assembler(...)`) wires the per-project
  ...
  pin records one version per dependency coordinate **per scope**, and every
  pin uses the single
  `@jenesis.pin` tag (in `module-info.java`) or `<!--jenesis.pin ... -->` comment block (in `pom.xml`).
  Scope is the single isolation axis: the canonical properties
  key is always `<scope>/<repository>/<coordinate>` (e.g.
  `compile/module/org.junit.jupiter`,
  `kotlin/maven/org.jetbrains.kotlin/kotlin-compiler-embeddable`), so the project's own dependencies (the
  `compile` and `runtime` scopes) and a tool's copy of the same library (a separate scope - the Kotlin/Scala/Groovy
  compiler in its `kotlin`/`scala`/`groovy` scope, or an
  `InternalModule` / `ExternalModule` build tool in `compile`) never collide. A pin tag is always scope-first with no
  shortcuts: the grammar is `@jenesis.pin <scope>/<repository>/<coordinate> <version> [<algo>/<hash>]`. The
  repository is spelled out (`module`, `maven`); a `module-info.java` can therefore
  pin a Maven coordinate it pulls in transitively (e.g. a non-modular transitive of a named module dependency)
  as `compile/maven/<group>/<artifact>` even though its declared dependencies resolve through the `module`
  repository. Scopes whose resolution Maven must not act on (the compiler scopes and any
  `plugin:<compiler>` scope) stay out of
  ...
  compiler modules resolve into the `kotlin`, `scala`, and `groovy` scopes respectively, so the inferred compiler
  chain isolates them with no extra wiring. Because each compiler resolves in its own scope, the compiler
  process is launched against only its own scope's artifacts, so a project that pins a different version of a
...
- **A module's exposed steps must not publish the same file at the same relative path in more than one of
  ...
  by name but disappears from the module's published map. (`Resolve` sidesteps this by design:
  `Resolve` writes its resolved closure to a distinct `transitives.properties` rather than overwriting the
...
- **Define each step-name constant once, at the class that adds the step**, and have all consumers reference
  ...
  because that's the framework that wires those sub-modules; `DependenciesModule.ARTIFACTS`
  belongs on `DependenciesModule` because that's where the step is added. The per-scope sub-module folder
...
- **Schema-level scope tokens are matched as literal strings.** The scope segment that leads every dependency
  ...
  Beyond the two everyday scopes a key may carry a tool scope: `plugin` (a javac annotation processor),
  `plugin:<compiler>` (a language compiler plugin, e.g. `plugin:kotlin`), or a language compiler's own scope
  (`kotlin`, `scala`, `groovy`); `Inventory` keeps every non-`compile`/`runtime` scope out of the
...
| `dependencies/`            | `BuildStep.DEPENDENCIES`         | A flat directory holding **downloaded dependency jars** that `Resolve` placed for this step (every transitive jar pulled in for the configured scope, whether external Maven or a sibling module's binary that was resolved by coordinate). Downstream classpath/module-path consumers (`Javac`, `Java`, `Javadoc`, `TestEngine`) walk both `artifacts/` and `dependencies/` to assemble the full set of jars. |
...
| `requires.properties`      | `BuildStep.REQUIRES`             | `<group>/<scope>/<repository>/<coordinate>` keys (the `identity.properties` `<repository>/<coordinate>` shape with leading group and scope segments; the group names the resolver closure the producer assigns - `main` for the project's own `compile`/`runtime` dependencies, the tool name for each compiler/plugin closure - so every scope sharing a group is mediated into one shared version tree), mapped to either an empty value (no integrity validation requested) or an `<algorithm>/<hex>` content checksum that `Resolve` verifies against the downloaded artifact (mismatch fails the build). Checksums are pinned in source by the user: a `<!--Checksum/<algorithm>/<hex>-->` comment inside a POM `<dependency>` element, or a `<!--Checksum/...-->` inside `<dependencyManagement>` (which propagates to whichever transitive resolves to that coord), or an `@jenesis.pin <scope>/<repository>/<coordinate> <version> <algorithm>/<hex>` Javadoc tag in `module-info.java`. Checksums are computed once, by the `pin` step: `PinPom` / `PinModuleInfo` read each module's resolved closure from `inventory.properties` (`prefix.dependency.<n>`, which carries each coordinate's self-anchored jar path), rehash the referenced jar using `-Djenesis.project.digest` (default `SHA-256`), and write the result back into `pom.xml`'s `<!--Checksum/...-->` comments or `module-info.java`'s `@jenesis.pin <scope>/<repository>/<coordinate> <version> <algorithm>/<hex>` Javadoc tags. `Resolve` then validates every subsequent fetch against the pinned checksum (mismatch fails the build); a coordinate that still has no pinned checksum is downloaded without integrity validation - or, when strict pinning is enabled (the `-Djenesis.dependency.pin=strict` property resolved as `Project`'s default pinning via `Pinning.fromProperty()`, or an explicit `Project.pinning(Pinning.STRICT)` override, threaded through `MavenProject.make` / `ModularProject.make` / `DependenciesModule` / `ProjectModuleDescriptor` / `JavaMultiProjectAssembler` / `TestModule` into every `Resolve`), the build fails. `Resolve` consumes the group and writes the resolved transitive closure to `transitives.properties` (not back into `requires.properties`); there, module-style coordinates carry an optional trailing `/<version>` segment (`compile/module/org.junit.jupiter/5.11.3`) reflecting the version a resolver chose for that module. |
| `transitives.properties`   | `BuildStep.TRANSITIVES`          | The resolved transitive closure that `Resolve` writes from `requires.properties`. Keys are scope-only `<scope>/<repository>/<coordinate>` (the resolver group is an input to resolution only and is dropped here), with the same per-coordinate checksum values as the declared `requires.properties`. `Versions` and `Inventory` read it; keeping it under a distinct name means a declared `requires.properties` and a resolved closure are never the same file at two pipeline stages. |
| `versions.properties`      | `BuildStep.VERSIONS`             | `<group>/<scope>/<repository>/<version-less-coordinate>=<version>[ <algorithm>/<hex>]` entries that act as a *bill of materials* for the resolution that follows: every resolver receives this map alongside `requires.properties` and uses the version part to pin any (declared or transitive) dependency that matches the bare coordinate. The optional space-separated `<algorithm>/<hex>` suffix is the pre-pinned content checksum for that coordinate; resolvers carry it through into the resolved `transitives.properties` value so `Resolve` validates the bytes against it. For Maven the coordinate is `groupId/artifactId[/type[/classifier]]`; for modules it is the bare Java module name. The leading group keeps an independent resolution closure (e.g. a `kotlin` compiler closure) separate from the project's own `main` group, so each group resolves and pins on its own. The file is written next to `requires.properties` by producers that have version data to contribute (`ModularProject` from `@jenesis.pin` Javadoc tags, `MavenProject` from `<dependencyManagement>` and the `<!--jenesis.pin-->` comment block). |
...
| `Resolve`                  | Reads `requires.properties` and (when present) `versions.properties`, groups the keys by their leading resolver-group segment, then asks each group's `Resolver` for the transitive closure with the version map as a pin set, materializes (downloads or links) each resolved jar into `dependencies/`, and writes the resolved coordinates to `transitives.properties` (module-style coordinates pick up a trailing `/<version>` segment when a version is known; the group segment is dropped, so the resolved file is scope-only). Checksums supplied via the `versions.properties` `version checksum` suffix - or via comments in transitive POMs the resolver visits - are propagated as the value for matching resolved coordinates and validated against the materialized bytes (mismatch fails the build); strict pinning is applied via the `.pinning(...)` builder. It also reads `exclusions.properties` (when present) to drop excluded transitives during resolution, and writes `dependencies.properties`, the `<scope>/<repository>/<coordinate>`-to-jar index. | `requires.properties`, `versions.properties`, `exclusions.properties`                                                                  | `transitives.properties`, `dependencies/<repository>-<coordinate>.jar`, `dependencies.properties`             |
| `Versions`                 | Walks each predecessor's `classes/`, hard-links every non-`module-info.class` file under `context.next()/classes/`, and rewrites every `module-info.class` so each `requires <X>` directive gets a `compiledVersion` set from the matching entry in `transitives.properties` (module-style `<prefix>/<name>/<version>` coordinates). Uses the JDK's `java.lang.classfile` API; module flags (`OPEN`), the module's own version, `exports`, `opens`, `uses` and `provides` round-trip unchanged. | `classes/`, `transitives.properties`                                                                                                     | `classes/` (non-`module-info` hard-linked, `module-info.class` rewritten in-place) |
...
| `PinPom`                   | Reads the module's resolved closure from `inventory.properties` (the `prefix.dependency.<n>` entries, each a concrete `<scope>/<repository>/<coordinate>/<version>` coordinate with its self-anchored jar path and any `<algorithm>/<hex>` checksum), excluding the project's own coordinates advertised in every module's `prefix.identity`, selects the `compile`/`runtime`-scope entries that resolve through the `maven` repository, and rewrites the configured `pom.xml` source file(s) so that the `<dependencyManagement>` block lists every entry as a `<dependency>` (with `<type>`/`<classifier>` when present) plus a `<!--Checksum/<algorithm>/<hex>-->` comment when one is known. The descriptor's own managed `versions.properties` is **not** read back in, so a module is pinned strictly from what it actually resolves and stale entries are dropped. Where the jar a `prefix.dependency` entry points at is present its checksum is recomputed; otherwise the entry's carried checksum is used. Replaces the existing block in place if present, inserts one before `<dependencies>` (or before `</project>`) if absent. Also strips any `<!--Checksum/...-->` comments from direct `<dependency>` entries outside `<dependencyManagement>`, since the rewritten BOM is the single source of truth for those checksums. Entries in a tool scope (the compiler scopes, `plugin:<compiler>`) are written instead into a Maven-ignored `<!--jenesis.pin ... -->` comment block (carrying the full `<scope>/<repository>/<coordinate>` key, `--` stored as `&#45;&#45;`), so `<dependencyManagement>` stays the project's own dependencies. Accepts either a single `Path` or a `List<Path>` of pom.xml files to update. Always re-runs (`shouldRun = true`) and writes back to the source file outside `context.next()`. | `inventory.properties` (`prefix.dependency` + `prefix.identity` + `prefix.path`) from each predecessor                              | none under `context.next()`; mutates the configured `pom.xml` file(s)            |
| `PinModuleInfo`            | Reads the module's resolved closure from `inventory.properties` (the `prefix.dependency.<n>` entries with their self-anchored jar paths), excluding the project's own coordinates from every module's `prefix.identity`, and rewrites the configured `module-info.java` source file(s) so that the preceding Javadoc block contains `@jenesis.pin <scope>/<repository>/<coordinate> <version>[ <algorithm>/<hex>]` tags for every entry. Tool-scope coordinates (compiler / plugin / internal / external scopes) are always pinned, with checksums recomputed from the jar each `prefix.dependency` entry references. In `MODULAR_TO_MAVEN` (`fromJars` mode) the `compile`/`runtime`-scope entries that resolve through the `module` repository are pinned by inspecting that same jar for its real module name, but only for jars that are genuine modules (a `module-info` or an `Automatic-Module-Name`); plain, non-modular jars resolved through `maven` are pinned under their `compile/maven/<group>/<artifact>` key instead of a filename-derived synthetic module name that would fail to re-resolve. Replaces the existing `@jenesis.pin` lines in place if present (preserving other block tags like `@jenesis.release`, `@jenesis.test`); inserts a fresh Javadoc block above the module declaration if none exists. Every tag is written scope-first as `@jenesis.pin <scope>/<repository>/<coordinate> <version>[ <algorithm>/<hex>]`. Accepts either a single `Path` or a `List<Path>` of module-info.java files. Always re-runs (`shouldRun = true`) and writes back to the source file outside `context.next()`. | `inventory.properties` (`prefix.dependency` + `prefix.identity` + `prefix.path`) from each predecessor                              | none under `context.next()`; mutates the configured `module-info.java` file(s)   |
...
step parses the dependency files into `(group, scope, repository, coordinate)` entries: `Resolve` reads
`requires.properties`, writes the resolved closure to `transitives.properties`, materializes each jar into
`dependencies/`, and writes the `dependencies.properties` index.
...
an inlined `Resolve` step (wired through a nested `DependenciesModule`), so the user never has to declare it as a compile-time `requires`
...
- The nested `dependencies` (`DependenciesModule`) takes the runner coordinate *together* with the project's already-transitively-resolved
  compile/runtime deps (carried from the upstream `transitives.properties`) and runs the
  resolve a second time across the combined set, materializing the unified set in one `Resolve` step. The resolver dedups by coordinate key and negotiates a
...
- The nested `dependencies`' `artifacts` (`Resolve`) step materializes the unified resolved set into `dependencies/`, validating checksums when
...
flowchart LR
  ...
  artifacts["dependencies/artifacts<br/>(Resolve)"]:::step
...
Used for resolving and downloading external dependencies declared in `requires.properties`. A single `Resolve`
step (wired under the name `artifacts`) computes the transitive closure and materializes the jars. `Resolve` validates each downloaded jar against the
checksum recorded in `transitives.properties` when one is present (sourced from POM/`module-info` pins, see
...
flowchart LR
  ...
  artifacts["artifacts (Resolve)<br/>writes transitives.properties,<br/>dependencies/, dependencies.properties"]:::step
  deps --> artifacts
```
...
`Resolve` validates non-empty `transitives.properties` values against the downloaded bytes and fails the build on
...
`prepare` (`MultiProjectDependencies`), `dependencies` (`DependenciesModule`: a single `Resolve` step),
...
`@jenesis.pin <scope>/<repository>/<coordinate> <version>`
...
resolves into the `plugin` scope; `@jenesis.plugin <compiler> <repository>/<coordinate>` names a compiler first
(`@jenesis.plugin kotlin maven/org.jetbrains.kotlin/kotlin-serialization-compiler-plugin`) and resolves into the
`plugin:<compiler>` scope (here `plugin:kotlin`). No entry point is named - annotation processors (`META-INF/services`), Kotlin
...
it under the `plugin` scope (`DependencyScope.PLUGIN`) or the `plugin:<compiler>` scope. A `plugin`-scope
dependency that is also a `requires static` is still pinned independently under both scopes. The plugin scopes
resolve and
...
its own plugins **by scope**: the Java compiler takes
the `plugin`-scope jars and hands them to `javac` as an explicit processor path; the Kotlin
compiler takes the `plugin:kotlin`-scope jars; the Scala compiler takes `plugin:scala`. This is what lets the scope
 ...
 * @jenesis.pin compile/module/org.immutables.value 2.12.2 SHA-256/abcdef0123...
...
Each compiler routes its share of the plugin scopes to that compiler's plugin flag: `javac` (the `plugin` scope) to the processor
path, the Kotlin compiler (`plugin:kotlin`) to `-Xplugin=<jar>`, the Scala compiler (`plugin:scala`) to `-Xplugin:<jar>`. Each compile step matches
...
(e.g. `@jenesis.pin runtime/module/org.junit.jupiter 5.11.3 SHA256/abcdef0123...`) is captured into the same
`versions.properties` value (as `version checksum`); the resolver propagates it to whichever transitive
resolves to that bare module name, so `Resolve` validates the bytes against it. There is no on-the-fly
...
(`MultiProjectDependencies`), `dependencies` (`DependenciesModule`: a single `Resolve` step),
`build` (caller-supplied, typically `JavaToolchainModule`), `assign` (`Assign`), and `inventory` (`Inventory`,
producing the per-module `inventory.properties` consumed by `Execute`) for each project.
...
- `coordinate` writes the requested coordinate (plus any added via `dependencies(...)`) into a fresh
  ...
  so the project's `<scope>/<repository>/<coordinate> <version>
...
- `compile-requires` and `runtime-requires` each parse the source's `module-info.java` and write a
  ...
  as the plug-in's `versions.properties`, so the project's `<scope>/<repository>/<coordinate> <version> <checksum>` pins
...
`Resolve` step validates against. `MODULAR_TO_MAVEN` wires `MavenModuleResolver` directly under the
...
`Resolve` always asks for `<module>/<version>`, never the bare name - so the root mirror's
...
- **`ModuleInfoParser` / `ModuleInfo`** parse `module-info.java` via the `javax.tools` / `com.sun.source` APIs
  ...
  `requires static transitive`), and a `versions` map extracted from `@jenesis.pin <scope>/<repository>/<coordinate> <version>` Javadoc tags
...
- **`MavenPomResolver`** is the resolver. Implements `Resolver` and is the densest piece of code in the repo:
  ...
  coordinates that the `Resolve` step consumes.
