...
- **A module's exposed steps must not publish the same file at the same relative path in more than one of
  ...
  by name but disappears from the module's published map. (`Resolve` -> `Download` sidesteps this by design:
...
| `dependencies.properties`  | `BuildStep.DEPENDENCIES_INDEX`   | Written by `Download` next to the `dependencies/` it fills: a single index mapping each resolved `<scope>/<repository>/<coordinate>` (with its trailing `/<version>` segment) to the **self-anchored relative path** of the jar it produced (`dependencies/<scope>-<repository>-<coordinate>.jar`). The leading scope is what isolates each resolution closure - a compiler's `kotlin/...` jars and the project's own `compile/...` jars sit in the same index but never collide. It is the authoritative coordinate-to-artifact index, so consumers never reconstruct a jar filename from a coordinate: `Inventory` joins it with `transitives.properties` (checksum) to assemble both the runtime closure (`prefix.runtime`, the `compile`/`runtime` scopes only) and the per-module dependency closure (`prefix.dependency`, every scope) it publishes, and `pin` then hashes the jar each `prefix.dependency` entry references rather than reading this file itself. Consumers read each value with `<file's parent folder>.resolve(value).normalize()`. |
...
| `Resolve`                  | Reads `requires.properties` and (when present) `versions.properties`, groups the keys by their leading resolver-group segment, then asks each group's `Resolver` for the transitive closure with the version map as a pin set, and writes the resolved coordinates to `transitives.properties` (module-style coordinates pick up a trailing `/<version>` segment when a version is known; the group segment is dropped, so the resolved file is scope-only). Checksums supplied via the `versions.properties` `version checksum` suffix - or via comments in transitive POMs the resolver visits - are propagated as the value for matching resolved coordinates, so the downstream `Download` step can validate them. It also reads `exclusions.properties` (when present) to drop excluded transitives during resolution. | `requires.properties`, `versions.properties`, `exclusions.properties`                                                                  | `transitives.properties` (transitively resolved closure, per-group `Resolver`)             |
| `Download`                 | Reads `transitives.properties` and downloads each coordinate's artifact into `dependencies/`, validating against the recorded checksum where present (mismatch fails the build) and reusing a previous run's file when valid. Coordinates with empty values are downloaded without integrity validation. Writing to `dependencies/` (instead of `artifacts/`) keeps the module's own produced binary separated from its downloaded deps. Writes `dependencies.properties`, the `<scope>/<repository>/<coordinate>`-to-jar index (the scope is carried on each key, so a tool closure such as a `kotlin` compiler is indexed apart from the project's own `compile`/`runtime` jars). | `transitives.properties`                                                                                                                 | `dependencies/<scope>-<repository>-<coordinate>.jar`, `dependencies.properties` |
...
is the shared interface implemented by `Resolve`, `Download`, and `Translate`; they all parse the dependency files into
`(group, scope, repository, coordinate)` entries and transform them - `Resolve` reads `requires.properties` and writes the
resolved closure to `transitives.properties`, `Download` reads `transitives.properties`, and `Translate` rewrites keys in place.
...
- `required` (`Resolve`) takes the runner coordinate *together* with the project's already-transitively-resolved
...
flowchart LR
  ...
  artifacts["artifacts (Download)<br/>writes dependencies/,<br/>dependencies.properties"]:::step
  deps --> resolved
  resolved --> artifacts
```
...
`Download` validates non-empty `transitives.properties` values against the downloaded bytes and fails the build on
