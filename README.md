...
`Dependencies` validates non-empty `dependencies.properties` checksums against the fetched bytes and fails the build on
mismatch; coordinates without a pinned checksum are fetched without integrity validation. There is no
