...
| `Stage`                    | Walks predecessors whose name ends in `/assign`, reads each `coordinates.properties`, and hard-links every resolved artifact into a per-module folder, producing a tree ready to publish or hand off.       | per `/assign`-named predecessor: its `coordinates.properties` (with non-empty values pointing at built artifacts)                       | for single-segment coordinates (e.g. `module/<jpms-name>`): `<jpms-name>/<artifact-filename>`. For Maven-shaped coordinates (`g/a/[t/[c/]]v`): `<groupId>/<artifactId>/<artifact-filename>`, with classifier'd entries getting a `<classifier>-` filename prefix (`tests` becomes `test-` to match Maven's `test-classes/` directory convention). |
