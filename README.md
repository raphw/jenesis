...
read in `doMain`, after the project is constructed, so it drives everything decided at run time (for example
`jenesis.project.watch` below); the fields the constructor fixes up front - layout, target, cache, pinning -
