...
steps parse the dependency files into `(group, scope, repository, coordinate)` entries: `Resolve` reads
`requires.properties` and writes the resolved closure to `transitives.properties`, which `Download` then reads.
...
`Download` step validates against. `MODULAR_TO_MAVEN` wires `MavenModuleResolver` directly under the
