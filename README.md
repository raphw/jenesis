...
| `Stage`                    | Walks predecessors whose name ends in `/assign`, reads each `coordinates.properties`, and hard-links every resolved artifact into a per-coordinate folder, producing a tree ready to publish or hand off. | per `/assign`-named predecessor: its `coordinates.properties` (with non-empty values pointing at built artifacts)                       | `<coordinate-suffix>/<artifact-filename>` for every resolved coordinate          |
