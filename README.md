...
- `MultiProjectAssembler<D extends ModuleDescriptor>` (set via `Project.assembler(...)`) wires the per-project
  ...
  through the `module` repository. Groups whose resolution Maven must not act on (the compiler groups, including their
...
integrity story extends to POMs the build pulls in for reference, not just to artifact jars. All of these
checksum comments are honoured only when they appear in the project's own POMs; a `<!--Checksum/...-->` comment
in a fetched transitive POM is ignored, so a third-party dependency cannot pin (and thereby self-certify) the
artifacts or POMs it pulls in.
