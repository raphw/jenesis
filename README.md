...
- `MultiProjectAssembler<D extends ModuleDescriptor>` (set via `Project.assembler(...)`) wires the per-project
  ...
  `kotlin/maven/org.jetbrains.kotlin/kotlin-compiler-embeddable`), so the project's own dependencies (group
  `main`, which mediates its `compile` and `runtime` scopes into one version tree) and a tool's copy of the same
  library (a separate group - the Kotlin/Scala/Groovy compiler in its `kotlin`/`scala`/`groovy` group, or an
  ...
  through the `module` repository. Groups whose resolution Maven must not act on (the compiler groups and any
  `plugin:<compiler>` group) stay out of
  ...
  compiler modules resolve into the `kotlin`, `scala`, and `groovy` groups respectively, so the inferred compiler
...
it under the `plugin` group or the `plugin:<compiler>` group (the group sits above the `compile`/`runtime` scope axis). A `plugin`-scope
