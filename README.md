...
compiles the image in one pass:
    ...
    java -Djenesis.observe.native=true -Djenesis.java.native=true build/jenesis/Project.java
...
so each image gets exactly its own test's metadata - no `META-INF/native-image/` commit in between. The tool is located through `GRAALVM_HOME`, then `java.home`, then `PATH`, and the executable is
written under the package phase's output:

    target/build/maven/compose/module/package/module-/package/native-image/output/native/build.jenesis
