...
| `compile-requires.properties` / `runtime-requires.properties` | `BuildStep.COMPILE_REQUIRES` / `BuildStep.RUNTIME_REQUIRES` | Per-scope variants of `requires.properties` written by the `Manifests` steps in `ModularProject` and `MavenProject` directly into the manifests output (no `compile/` or `runtime/` subdirs). Same key/value shape as `requires.properties`. The compile variant lists everything visible at compile time (plain `requires`, `requires static`, `requires transitive`, plus Maven `compile`/`provided`); the runtime variant lists what's also visible at runtime (omits `requires static` and Maven `provided`). `Pom` reads both to decide whether each dependency is emitted as `compile`, `provided`, or `runtime` scope; `MultiProjectDependencies` reads the appropriate one for its scope. |
| `compile-versions.properties` / `runtime-versions.properties` | `BuildStep.COMPILE_VERSIONS` / `BuildStep.RUNTIME_VERSIONS` | Per-scope variants of `versions.properties`. Same shape and role as `versions.properties` but partitioned by scope, written and read in the same flat layout as the corresponding requires files. |
