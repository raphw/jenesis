...
- `MultiProjectAssembler<D extends ModuleDescriptor>` (set via `Project.assembler(...)`) wires the per-project
  ...
  modules never leak into the dependency-management block. Within a module, pin records one version per
  dependency coordinate **per resolution trail**. Independent trails are distinguished by a qualifier on the
  coordinate prefix (`maven@kotlin/org.jetbrains.kotlin/kotlin-compiler-embeddable`): the unqualified trail holds
  the project's own dependencies (pinned in `<dependencyManagement>` for Maven, `@jenesis.pin` for modular),
  while a qualified trail - for example the Kotlin or Scala compiler, or an `InternalModule` / `ExternalModule`
  build tool - resolves on its own and is pinned separately, so a coordinate the project depends on at one
  version and the toolchain resolves at another no longer collide. Qualified pins are stored where Maven ignores
  them but jenesis can retrieve them, in both cases as `[<prefix>/]<qualifier>@<coordinate> <version>
  [<algorithm>/<hex>]` entries whose leading `<prefix>/` (the resolution namespace) is present only when a `/`
  precedes the first `@`, and otherwise falls back to a per-source default: in `pom.xml` a
  `<!--jenesis.requires ... -->` comment block (one entry per line, default prefix `maven`; because an XML
  comment cannot contain `--`, any such sequence in a value is stored as `&#45;&#45;`), and in
  `module-info.java` an `@jenesis.requires ...` Javadoc tag (default prefix `module`). So a module pin in a
  `module-info` carries no prefix while a Maven dependency is written `maven/<qualifier>@...`, and a second Maven
  source in a `pom.xml` is written `maven2/<qualifier>@...` while the primary one stays bare. The Kotlin and
  Scala compiler modules default their qualifier to `kotlin` and `scala` respectively (overridable via
...
A `@jenesis.requires [<prefix>/]<qualifier>@<coordinate> <version>[ <algorithm>/<hex>]` tag pins a coordinate on an
independent resolution trail (the prefix defaults to `module`, so a Maven dependency is written
`maven/<qualifier>@<groupId>/<artifactId>`); it is captured into the same `versions.properties` under its
qualified prefix and resolves separately from the module's own dependencies.
...
function suffices. `MODULAR_TO_MAVEN` does not use it; instead `MavenModuleResolver` plugs directly into
`ModularJarResolver` as a fallback, fetching each `module/<name>` coordinate's `:pom` from a
`JenesisModuleRepository(false)` and handing the bytes to `MavenPomResolver`'s pre-fetched-root entry
point so the first-layer POMs are never re-downloaded from Maven Central.
