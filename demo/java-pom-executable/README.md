...
`jpackage`. It is the executable counterpart of `../java-pom`, and its modular
sibling is `../java-modular-executable`.
...
`../java-modular-executable`). A POM project has no `module-info.java`, so the
