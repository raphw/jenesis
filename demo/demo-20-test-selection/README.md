...
change, and leave the rest cached. It is enabled with `-Djenesis.test.select=true`
and is meant mainly for **watching** a project during development, where the
build re-runs on every save and a fast, narrowed test pass keeps the feedback
loop tight:

    java -Djenesis.project.watch=true -Djenesis.test.select=true build/jenesis/Project.java
...
1. A first build (`-Djenesis.test.select=true`) has no previous snapshot, so it
