...
if not defined JAVA (
    where java >nul 2>&1
    if errorlevel 1 (
        echo jenesis-exec: no Java runtime found - set JAVA_HOME or add 'java' to PATH ^(Java 25 or newer required^) 1>&2
        exit /b 1
    )
    set "JAVA=java"
)
...
if not defined JAVA_VERSION (
    echo jenesis-exec: failed to determine Java version from '!JAVA! -version' 1>&2
    exit /b 1
)
...
"!JAVA!" %JAVA_OPTS% -p "!JENESIS_HOME!\lib" -m build.jenesis/build.jenesis.Execute %*
exit /b %ERRORLEVEL%
