...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$jr = $env:JENESIS_VALIDATE_JAR_ROOT; $cr = $env:JENESIS_VALIDATE_CUR_ROOT; $pp = $env:JENESIS_VALIDATE_PATH_PREFIX; $sp = $env:JENESIS_VALIDATE_SUMMARY_PREFIX; function H($p){(Get-FileHash -Algorithm SHA256 -LiteralPath $p).Hash.ToLower()}; $matched = 0; $differed = 0; if (Test-Path -LiteralPath $jr) { foreach ($f in Get-ChildItem -LiteralPath $jr -Recurse -File) { $rel = $f.FullName.Substring($jr.Length + 1); $relPosix = ($pp + 'build/jenesis/' + ($rel -replace '\\', '/')); $cur = Join-Path $cr $rel; $h1 = H $f.FullName; if (Test-Path -LiteralPath $cur) { $h2 = H $cur; if ($h1 -ne $h2) { Write-Output \"$relPosix $h1 $h2\"; $differed++ } else { $matched++ } } else { Write-Output \"$relPosix $h1 -\"; $differed++ } } }; if (Test-Path -LiteralPath $cr) { foreach ($f in Get-ChildItem -LiteralPath $cr -Recurse -File) { $rel = $f.FullName.Substring($cr.Length + 1); $relPosix = ($pp + 'build/jenesis/' + ($rel -replace '\\', '/')); $jp = Join-Path $jr $rel; if (-not (Test-Path -LiteralPath $jp)) { $h2 = H $f.FullName; Write-Output \"$relPosix - $h2\"; $differed++ } } }; Write-Output \"jenesis-validate: ${sp}$matched matched, $differed differed\""
exit /b !ERRORLEVEL!
